...
export const updateParty= (req, res) =>{
    try {
        const {partyName,partyDate,partyTime,partyLocation, partyType,partyDescription, partyPrice,partyImage,partyCreator,partyParticipants,createdAt,id} = req.body;
        console.log(req.body);
        if (!partyName || !id ||!partyDate || !partyTime || !partyLocation || !partyType || !partyDescription || !partyPrice ||!partyImage || !partyCreator || !partyParticipants || !createdAt) throw new Error("Please complete all fields");
